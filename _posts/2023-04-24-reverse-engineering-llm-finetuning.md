...
Second, **Reverse Engineering the fine-tuned model to understand the major prompts/instructions would be very helpful.** How would you do that is the major concern, **mechanistic interpretability** or asking the model itself or altogether using **agents**. Using Agents to divide the big problem into multiple small problems and using agents to solve each small problem and correcting based on the feedback seems to be the go to workflow now.

**unfortunately there is no straightforward solution to these, but the idea that knowing what went into fine-tuning the llm would definitely be very helpful in writing good prompts or even designing the agentic workflows. On a very high level, fine-tuning at this stage is similar to creation of api's in general software development. Pre-trained models have lot to say on the output of prompts, but fine-tuning also effects on the output of prompts. Exposing those api's is how we use those functionality.**
